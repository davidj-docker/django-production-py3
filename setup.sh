...
/bin/bash -c "source /.virtualenv/venv/bin/activate && pip3 install -r /app/requirements.txt"

if [ -e /app/requirements.txt ]
then
	/bin/bash -c "source /.virtualenv/venv/bin/activate && pip3 install -r /app/requirements2.txt"
fi

# Symlink admin static files
/bin/bash -c "ln -s /.virtualenv/venv/lib/python3.4/site-packages/django/contrib/admin/static/admin /app/$WORKING_DIRECTORY/$STATIC_DIRECTORY/admin"
